...
if(isset($_GET[regno]))
{
	...
	if(isset($_GET[examid]))
	{
		...
		//answered questions
		$sqlanswered = "SELECT * FROM results inner join certificate on certificate.examid=certificate.examid where results.examid=$_GET[examid] AND results.answerid<>0";
		$qanswered = mysqli_query($con,$sqlanswered);
		$rsanswered = mysqli_fetch_array($qanswered);
		
		//Correct answers
		$sqlexamresult = "SELECT * FROM results INNER JOIN questions ON questions.queid = results.queid where results.examid=$_GET[examid]";
		$qexamresult = mysqli_query($con,$sqlexamresult);
